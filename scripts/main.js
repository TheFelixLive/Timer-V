...
const version_info = {
  ...
  changelog: {
    // new_features
    new_features: ["All entities can now be set as a goal"],
    // general_changes
    general_changes: [
  ...
  },
  {
    ...
    show_if: (save_data, player_sd_index) => {
      return (
        ...
        save_data[player_sd_index].show_td_as_mode
      );
    },
  },
];
...
const design_template = [
  {
    ...
    content: [
        ...
        blocks: [
          {
            ...
            alwaysShow: false,
            suffix: "y",
            separator: { enabled: true, value: " ", position: "after" },
          },
          {
            type: "marker",
            marker: "d",
      ...
      },
      {
        ...
        blocks: [
          { type: "text", text: "Your time ->\n§l§7" },
          {
            ...
            padZero: false,
            alwaysShow: false,
            suffix: { singular: " year, ", plural: " years, " },
            separator: { enabled: false },
          },
          {
            type: "marker",
          ...
          },
          {
            ...
            marker: "h",
            padZero: false,
            alwaysShow: { condition: "ifGreater", units: ["y", "d"] },
            suffix: "",
            separator: { enabled: true, value: ":", position: "after" },
          },
          {
      ...
      { type: "screen_saver", blocks: [{ type: "text", text: "§lTimer §tV" }] },
    ],
  },
  {
    ...
    content: [
      ...
      {
        type: "screen_saver",
        blocks: [{ type: "text", text: "§lTimer §7V§r §f§oby TheFelixLive" }],
      },
    ],
  },
  {
...

  {
    ...
    content: [
      ...
      {
        type: "screen_saver",
        blocks: [{ type: "text", text: "§p§lTimer V §r§h§oLite§r" }],
      },
    ],
  },
  {
    ...
    content: [
      ...
      {
        type: "screen_saver",
        blocks: [{ type: "text", text: "§b§lTimer V §o§3by TheFelixLive" }],
      },
    ],
  },
        ...
        blocks: [
          {
            ...
            alwaysShow: true,
            suffix: "§eh",
            separator: { enabled: true, value: "§r ", position: "after" },
          },
          {
            type: "marker",
            marker: "m",
...
console.log(
  "Hello from " +
    ...
    " (" +
    version_info.build +
    ") - Further debugging is " +
    (version_info.release_type == 0 ? "enabled" : "disabled") +
    " by the version",
);
...
const default_save_data_structure = {
  ...
  counting_type: 0,
  challenge: {
    active: world.isHardcore ? true : false,
    progress: 0,
    rating: 0,
  ...
  global: {
    status: world.isHardcore ? true : false,
    last_player_id: undefined,
  },
  sync_day_time: false,
  utc: 0,
  update_message_unix:
    version_info.unix + version_info.update_message_period_unix,
};
...
function create_player_save_data(playerId, playerName, modifier) {
  ...
  for (let key in modifier) {
    if (player_data.hasOwnProperty(key)) {
      // Only apply modifier if the key exists in player_data
...
world.afterEvents.playerJoin.subscribe(async ({ playerId, playerName }) => {
  ...
  if (version_info.release_type !== 2) {
    player.sendMessage(
      "§l§7[§f" +
        (independent ? "System" : version_info.name) +
        "§7]§r " +
        save_data[player_sd_index].name +
        " how is your experiences with " +
        version_info.version +
        "? Does it meet your expectations? Would you like to change something and if so, what? Do you have a suggestion for a new feature? Share it at §lgithub.com/TheFelixLive/Timer-V",
    );
  ...
  if (
    save_data[player_sd_index].last_unix >
      Math.floor(Date.now() / 1000) + 604800 &&
    independent
  ) {
    player.sendMessage(
      "§l§6[§eHelp§6]§r You can always open the menu with the sneak-jump (or in spectator with the nod) gesture, with the command §l/scriptevent timerv:menu§r§f or with a stick",
    );
  }
  ...
  if (typeof gen === "number" && save_data[player_sd_index].op) {
    if (save_data[player_sd_index].setup == 0) {
      player.sendMessage(
        "§l§1[§9Update§1]§r We found save data from " +
          gen_list[gen] +
          "! You can convert them and use it in " +
          version_info.version,
      );
    } else {
      player.playMusic(translate_soundkeys("music.menu.setup", player), {
        fade: 0.3,
      });
...
function startup_popups(player) {
  ...
  // Update popup
  if (
    save_data[player_sd_index].op &&
    Math.floor(Date.now() / 1000) > save_data[0].update_message_unix
  ) {
    player.playMusic(translate_soundkeys("music.menu.setup", player), {
      fade: 0.3,
    });
    ...
    form.body(
      "Your current version (" +
        version_info.version +
        ") is now " +
        getRelativeTime(Math.floor(Date.now() / 1000) - version_info.unix) +
      ...
      form.show(player).then((response) => {
        if (response.canceled && response.cancelationReason === "UserBusy") {
          ...
          if (response.selection === 0) {
            save_data[0].update_message_unix =
              Math.floor(Date.now() / 1000) +
              version_info.update_message_period_unix;
            update_save_data(save_data);
          }
          if (response.selection == undefined) {
            return player.playMusic(translate_soundkeys("menu.close", player), {
              fade: 0.3,
            });
          }
        }
...
function universel_updater(player, gen) {
  ...
  form.title("Convert");
  form.body(
    "It looks like you've used the timer before.\nDo you want to update your save data from " +
      gen_list[gen] +
      " to " +
      version_info.version +
...
function uu_apply_gen(gen, player) {
  ...
  if (gen == 0) {
    note_message =
      "Challenge settings could not be transferred because they are not currently supported";
    const getScoreSafe = (objective, scoreName, defaultValue = 0) => {
      try {
        return objective.getScore(scoreName) || defaultValue;
      } catch (error) {
        console.error(
          `Error getting '${scoreName}' from '${objective}': `,
          error,
        );
        return defaultValue;
      }
    ...
    let stopwatchTime =
      getScoreSafe(settings, "shoud_count_down") === 0
        ? getScoreSafe(time, "ms") / 5 +
          getScoreSafe(time, "sec") * 20 +
          getScoreSafe(time, "min") * 20 * 60 +
          getScoreSafe(time, "h") * 20 * 60 * 60
        : 0;

    let timerTime =
      getScoreSafe(settings, "shoud_count_down") === 1
        ? getScoreSafe(time, "ms") / 5 +
          getScoreSafe(time, "sec") * 20 +
          getScoreSafe(time, "min") * 20 * 60 +
          getScoreSafe(time, "h") * 20 * 60 * 60
    ...
    let save_data = [
      {
        time: {
          ...
          timer: timerTime,
          last_value_timer: timerTime,
          do_count: getScoreSafe(settings, "do_count") === 1,
        },
        ...
        challenge: {
          ...
          goal: {
            pointer: old_goal === 8 ? 0 : old_goal < 5 ? 1 : 2,
            entity_id: [
    ...
    world.getAllPlayers().forEach((player) => {
      try {
        create_player_save_data(player.id, player.name, {
          ...
          visibility_setting:
            show_actionbar.getScore(player) === 1 ? true : false,
        });
        if (player.hasTag("trust_player_control"))
          player.removeTag("trust_player_control");
      } catch (error) {
        console.error(
          `Error processing player data for player ${player.name}: `,
          error,
        );
      }
    });
  ...
  if (gen == 1) {
    ...
    function gen_1() {
      ...
      let old_global = world.scoreboard
        .getObjective("timer_menu")
        .getScore("host_mode");
      let do_count = world.scoreboard.getObjective("timer_do_count");
      let shoud_count_down = world.scoreboard
        .getObjective("timer_shoud_count_down")
        .getScore("host");
      let show_actionbar = world.scoreboard.getObjective(
        "timer_show_actionbar",
      );
      ...
      let save_data = [
        {
          time: {
            stopwatch: shoud_count_down
              ? 0
              : time_ms.getScore("host") / 5 +
                time_sec.getScore("host") * 20 +
                time_min.getScore("host") * 20 * 60 +
                time_h.getScore("host") * 20 * 60 * 60 +
                time_d.getScore("host") * 20 * 60 * 60 * 24,
            timer: shoud_count_down
              ? time_ms.getScore("host") / 5 +
                time_sec.getScore("host") * 20 +
                time_min.getScore("host") * 20 * 60 +
                time_h.getScore("host") * 20 * 60 * 60 +
                time_d.getScore("host") * 20 * 60 * 60 * 24
              : 0,
            last_value_timer: shoud_count_down
              ? time_ms.getScore("host") / 5 +
                time_sec.getScore("host") * 20 +
                time_min.getScore("host") * 20 * 60 +
                time_h.getScore("host") * 20 * 60 * 60 +
          ...
          global: {
            status: old_global == 1 ? true : false,
            last_player_id:
              world.getAllPlayers().find((player) => {
                player.hasTag("target_host");
              })?.id || player.id,
          },
      ...
      for (const player of world.getAllPlayers()) {
        try {
          create_player_save_data(player.id, player.name, {
            time: {
              do_count: do_count.getScore(player) == 1 ? true : false,
              timer: 0,
              last_value_timer: 0,
              stopwatch:
                time_ms.getScore(player) / 5 +
                time_sec.getScore(player) * 20 +
                time_min.getScore(player) * 20 * 60 +
                time_h.getScore(player) * 20 * 60 * 60 +
                time_d.getScore(player) * 20 * 60 * 60 * 24,
            },
...
function uu_gen_successfull(player, note_message) {
  ...
  form.show(player).then((response) => {
    ...
    if (response.selection == 1) {
      player.playMusic(translate_soundkeys("music.menu.dictionary", player), {
        fade: 0.3,
        loop: true,
      });
      return dictionary_about_version_changelog(
        player,
        convertUnixToDate(version_info.unix, save_data[0].utc),
      );
    }
    if (response.selection == 0) {
      player.playMusic(translate_soundkeys("music.menu.main", player), {
        fade: 0.3,
        loop: true,
      });
...
system.afterEvents.scriptEventReceive.subscribe((event) => {
  ...
  if (["timerv:reset"].includes(event.id)) {
    const notAvailableMsg = (id) =>
      `§l§7[§f` +
      (independent ? "System" : version_info.name) +
      `§7]§r ${id} is not available in stable releases!`;
    const noPermissionMsg = (id) =>
      `§l§7[§f` +
      (independent ? "System" : version_info.name) +
      `§7]§r ${id} could not be changed because you do not have permission!`;
...
-------------------------*/
  ...
  if (
    event.id === "timerv:menu" &&
    independent &&
    save_data[player_sd_index].gesture.command
  ) {
  ...
  if (event.id === "timerv:menu_soundkey") {
    ...
    player.playSound(translate_soundkeys("menu.open", player));
    player.queueMusic(
      translate_soundkeys("music.menu.settings.debug", player),
      { fade: 0.3, loop: true },
    );
...
world.beforeEvents.itemUse.subscribe((event) => {
  ...
  if (
    ...
    independent &&
    save_data[idx].gesture.stick
  ) {
    system.run(() => {
      event.source.playSound(translate_soundkeys("menu.open", event.source));
      event.source.playMusic(
        translate_soundkeys("music.menu.main", event.source),
      );
...
function translate_soundkeys(key, player) {
  ...
  if (
    save_data[0].challenge.active &&
    save_data[0].challenge.progress == 0 &&
    entry.challenge_progress_0
  ) {
...
function convert_global_to_local(disable_global) {
  let save_data = load_save_data();
  let player_save_data = save_data.findIndex(
    (entry) => entry.id === save_data[0].global.last_player_id,
  );
  ...
  if (player_save_data) {
    ...
    world.sendMessage(
      "§l§4[§cError§4]§r The time could not be synchronized with the player profile (" +
        save_data[0].global.last_player_id +
        ") and got deleted!",
    );
  }
...
function start_cm_timer() {
  ...
  if (save_data[0].challenge.goal.pointer == 0) {
    ...
    if (r < totalEntities) {
      ...
      const realIndex = goal_event.findIndex(
        (g) => g.name === chosenEvent.name,
      );
      ...
      save_data[0].challenge.goal.pointer === 1
        ? [
            { text: "Defeat the " },
            {
              translate:
                "entity." +
                save_data[0].challenge.goal.entity_id.replace(
                  /^minecraft:/,
                  "",
                ) +
                ".name",
          ...
          ? [
              {
                text:
                  "Survive: " +
                  apply_design(design, save_data[0].time.timer) +
                  "\n",
              },
            ]
          : [
              {
                text:
                  ...
                  goal_event[save_data[0].challenge.goal.event_pos].name +
                  "\n",
              },
            ];

    world.sendMessage({
      rawtext: [{ text: "§l§5[§dGoal§5]§r§f " }, ...parts],
    });
  }
...
function finished_cm_timer(rating, message) {
  ...
  // Have to be rawtext!
  const prefix = {
    text: rating === 1 ? "§l§2[§aGoal§2]§r " : "§l§4[§cEnd§4]§r ",
  };
  ...
  world.getAllPlayers().forEach((t) => {
    t.playSound(
      translate_soundkeys(
        rating == 1 ? "challenge.end.good" : "challenge.end.bad",
        t,
      ),
    );
    t.onScreenDisplay.setTitle(
      rating == 1 ? "§aYou Won!" : "§4Challenge has ended!",
    );
  });
  ...
  for (const player of world.getPlayers()) {
    ...
    if (configuration == "infinity") {
      ...
      if (save_data[0].challenge.difficulty == 3) {
        player.applyDamage(
          health.currentValue - save_data[player_sd_index].health,
        );
...
function render_task_list() {
  ...
  if (save_data[0].challenge.goal.pointer === 1) {
    ...
    lines.push({
      translate:
        "entity." +
        save_data[0].challenge.goal.entity_id.replace(/^minecraft:/, "") +
        ".name",
    });
    lines.push({ text: "\n" });
  }
  ...
  if (
    ...
    lines.push({
      text:
        "- §aSurvive:§f " +
        getRelativeTime(save_data[0].time.timer / 20) +
        "\n",
    });
  } else {
    // time available
...
world.afterEvents.dataDrivenEntityTrigger.subscribe((eventData) => {
  ...
  if (
    ...
    triggerId === "minecraft:start_celebrating" &&
    save_data[0].challenge.progress == 1 &&
    save_data[0].time.do_count &&
    save_data[0].challenge.goal.pointer == 2 &&
    save_data[0].challenge.goal.event_pos == 1
  ) {
    finished_cm_timer(1, [
      {
        text: "You did it! You prevented the Raid and protected the village like a hero. Good Game!",
      },
    ]);
  }
});
...
world.afterEvents.entityDie.subscribe((event) => {
  ...
  if (event.deadEntity?.typeId === "minecraft:player") {
    ...
    const player_sd_index = save_data.findIndex(
      (entry) => entry.id === player.id,
    );
    ...
    if (
      ...
      finished_cm_timer(0, [
        {
          text:
            "The challenge is over. Time invested: " +
            apply_design(
              ...
              save_data[0].counting_type == 0
                ? save_data[0].time.stopwatch
                : save_data[0].time.last_value_timer - save_data[0].time.timer,
            ) +
            " Thanks for playing.",
        },
      ]);
  ...
  if (
    ...
    save_data[0].time.do_count &&
    save_data[0].challenge.goal.pointer == 1 &&
    event.deadEntity?.typeId === save_data[0].challenge.goal.entity_id
  ) {
    finished_cm_timer(1, [
      { text: "You did it! You defeated the " },
      {
        translate:
          "entity." +
          save_data[0].challenge.goal.entity_id.replace(/^minecraft:/, "") +
      ...
      { text: " in an epic battle! Good Game!" },
    ]);
  }
});
...
function apply_design(design, time) {
  ...
  const proc = (design.blocks || []).map((b) => {
    if (b.type === "marker") {
      ...
      if (num !== 0) show = true;
        ...
        typeof b.alwaysShow === "object" &&
        b.alwaysShow.condition === "ifGreater"
      ) {
        const anyGreater = (b.alwaysShow.units || []).some(
          (u) => (timeValues[u] || 0) > 0,
        );
        show = anyGreater;
      }
  ...
  const allZero = !proc.some(
    (b) => b.type === "marker" && Number(b.value) !== 0,
  );
  if (allZero) {
    proc.forEach((b, i) => {
  ...
  for (let i = 0; i < proc.length; i++) {
    ...
    if (b.type === "text") {
      ...
      if (
        b.separator?.enabled &&
        ["before", "both"].includes(b.separator.position)
      ) {
        const prev = proc
          .slice(0, i)
          .some((x) => x.type === "marker" && x.show);
      ...
      if (
        b.separator?.enabled &&
        ["after", "both"].includes(b.separator.position)
      ) {
        const next = proc
          .slice(i + 1)
          .some((x) => x.type === "marker" && x.show);
  ...
  if (
    design.type === "day" &&
    Array.isArray(design.colorConfig) &&
    design.colorConfig.length >= 3
  ) {
    const h = Number(timeValues.h),
      m = Number(timeValues.m);
    const tot = h * 60 + m;
    const color =
      tot < 4 * 60 + 30 || h >= 19
  ...
  if (version == 1 || version == 2) {
    ...
    for (const key in soundkeys) {
      ...
      if (version == 2 ? sound.extern : sound.extern_l) {
        form.button(
          version == 2 ? sound.extern : sound.extern_l,
          "textures/ui/sound_glyph_color_2x",
        );
        actions.push(() => {
          player.playMusic(version == 2 ? sound.extern : sound.extern_l, {
            fade: 0.3,
          });
          player.queueMusic(
            translate_soundkeys("music.menu.settings.debug", player),
      ...
      for (const subKey in sound) {
        ...
        if (version == 2 ? subsound.extern : subsound.extern_l) {
          form.button(
            version == 2 ? subsound.extern : subsound.extern_l,
            "textures/ui/sound_glyph_color_2x",
          );
          actions.push(() => {
            player.playMusic(
...
function main_menu_actions(player, form) {
  ...
  if (
    !save_data[0].global.status ||
    (save_data[0].global.status && save_data[player_sd_index].op)
  ) {
    if (timedata.counting_type == 0 || timedata.counting_type == 1) {
      if (
        ((timedata.counting_type == 0 ||
          (timedata.counting_type == 1) & (timedata.time.timer > 0)) &&
          (!save_data[player_sd_index].afk ||
            save_data[0].global.status ||
            timedata.time[
              timedata.counting_type == 1 ? "timer" : "stopwatch"
            ] == 0) &&
          !save_data[0].challenge.active) ||
        (save_data[0].challenge.active &&
          save_data[0].challenge.progress == 1 &&
        ...
        if (form) {
          form.button(
            "Condition" +
              (world.isHardcore ? " §o(experimental)§r\n" : "\n") +
              (timedata.time.do_count === true ? "§aresumed" : "§cpaused"),
        ...
        actions.push(() => {
          player.playMusic(translate_soundkeys("menu.close", player), {
            fade: 0.3,
          });
          if (timedata.time.do_count === false) {
            ...
            (save_data[0].global.status
              ...
              if (world.isHardcore) {
                save_data[player_sd_index].health =
      ...
      if (
          ...
          if (form) {
            form.button(
              "Intelligent condition\n" +
                (save_data[player_sd_index].afk === true ? "§aon" : "§coff"),
        ...
        if (form) {
          form.button(
            "§cReset " + (timedata.counting_type == 1 ? "timer" : "stopwatch"),
    ...
    if (
      ...
      ((isHardcore && challenge.rating === 1) || !isHardcore)
    ) {
      if (form)
        form.button("§aStart over!", "textures/ui/recap_glyph_color_2x");
    ...
    if (challenge.active && challenge.progress == 0) {
      ...
      actions.push(() => {
        player.playMusic(translate_soundkeys("music.menu.goal", player), {
          fade: 0.3,
          loop: true,
        });
      ...
      if (form)
        form.button(
          ...
          difficulty[challenge.difficulty].icon,
        );
      actions.push(() => {
        player.playMusic(translate_soundkeys("music.menu.difficulty", player), {
          fade: 0.3,
          loop: true,
        });
        ...
        form.button(
          "Shared timer\n§9" +
            (save_data[0].global.status
              ? "by " +
                save_data.find(
                  (e) => e.id === save_data[0].global.last_player_id,
                )?.name
              : "off"),
    ...
    if (
      !(challenge.active && challenge.progress > 0) &&
      timedata.counting_type !== 2
    ) {
      if (form) {
        form.button(
      ...
      actions.push(() => {
        player.playMusic(translate_soundkeys("music.menu.time", player), {
          fade: 0.3,
          loop: true,
        });
  ...
  if (
    (save_data[player_sd_index].time_day_actionbar == true ||
      timedata.counting_type == 3) &&
    save_data[0].challenge.progress !== 2
  ) {
    if (
      ...
      if (form) {
        form.button(
          "Clone real time\n" + (save_data[0].sync_day_time ? "§aon" : "§coff"),
          save_data[0].sync_day_time
            ? "textures/ui/toggle_on"
            : "textures/ui/toggle_off",
        );
      }
  ...
  if (
    ...
    if (form) {
      form.button(
        "Challenge mode\n" + (save_data[0].challenge.active ? "§aon" : "§coff"),
        save_data[0].challenge.active
          ? "textures/ui/toggle_on"
          : "textures/ui/toggle_off",
      );
...
function main_menu(player) {
  ...
  if (
    (actions.length == 1 && !save_data[0].challenge.active) ||
    (actions.length == 1 &&
      save_data[0].challenge.active &&
      save_data[0].challenge.progress !== 1)
  )
...
function splash_challengemode(player) {
  ...
  form.show(player).then((response) => {
    ...
    if (response.selection == 0) {
      ...
      (save_data[0].challenge.active = save_data[0].challenge.active
        ? false
        : true),
        update_save_data(save_data);
      player.playMusic(translate_soundkeys("music.menu.main", player), {
...
function splash_start_challenge(player) {
  ...
  form.body({
    rawtext: [
      {
        text: "You are trying to start a challenge. Once a challenge is started, many settings are no longer available.\n\nHere's a brief overview:\n",
      },
      ...render_task_list(),
      { text: "\n\n" },
    ],
...
function splash_end_challenge(player) {
  ...
  form.body({
    rawtext: [
      {
        text:
          "You are about to end a challenge.\nIf you complete the challenge this way, you will §llose all your progress" +
          (world.isHardcore ? " and this World!§r" : "§r."),
      },
    ],
  });
...
function splash_globalmode(player) {
  ...
  form.body(
    "The shared timer feature coppies your timer to an additional timer that is enforced on all players." +
      (save_data[0].global.status
        ? save_data[0].global.last_player_id !== player.id
          ? save_data.find((e) => e.id === save_data[0].global.last_player_id)
              ?.name +
            " is currently sharing his timer. You can §cstop§f this or §ereplace§f it with your own time (" +
            apply_design(
              design,
              save_data[player_sd_index].time[
                save_data[player_sd_index].counting_type == 1
                  ? "timer"
                  : "stopwatch"
              ],
            ) +
...
function settings_start_time(player) {
  ...
  const form = new ModalFormData().title(
    save[0].challenge.active ? "Start time" : "Change time",
  );
  ...
  function decomp(ms) {
    let y = Math.floor(ms / MS.year);
    ms %= MS.year;
    let dTot = Math.floor(ms / MS.day);
    ms %= MS.day;
    let w = Math.floor(dTot / 7),
      d = dTot % 7;
    let h = Math.floor(ms / MS.hour);
    ms %= MS.hour;
    let m = Math.floor(ms / MS.minute);
    ms %= MS.minute;
    let s = Math.floor(ms / MS.second);
    ms %= MS.second;
  ...
  form.show(player).then((res) => {
    ...
    if (
      ticks === 0 &&
      save[0].challenge.active &&
      save[0].challenge.goal.pointer === 2 &&
      save[0].challenge.goal.event_pos === 0
    ) {
...
function settings_difficulty(player) {
  ...
  difficulty.forEach((diff, index) => {
    if (
      diff.is_hardcore == world.isHardcore ||
      save_data[player_sd_index].allow_unnecessary_inputs
    ) {
      let name = diff.name;
  ...
  form.show(player).then((response) => {
    ...
    if (
      response.selection >= 0 &&
      response.selection < visibleDifficulties.length
    ) {
...
async function settings_goals_select(player, type) {
  ...
  const currentGoalID =
    !isEvent && !isRandom ? save_data[0].challenge.goal.entity_id : undefined;
  // Für Event-Typen weiterhin mit Index arbeiten
  const currentGoalIndex =
    isEvent && !isRandom ? save_data[0].challenge.goal.event_pos : undefined;
  ...
  if (isEvent) {
    ...
    visibleGoals = goalArray.filter((e) => {
      ...
      if (
        ...
        return false;
      if (goal_entity_blocklist.find((blocked) => blocked.id === baseId))
        return false;
      return true;
    });
  }
  ...
  visibleGoals.forEach((goal) => {
    ...
    const labelText = {
      rawtext: [
        isEvent
          ...
          : {
              translate:
                "entity." + goal.id.replace(/^minecraft:/, "") + ".name",
            },
        {
          text: (
            isEvent ? currentGoalIndex === realIndex : currentGoalID === goal.id
          )
            ? "\n§2(selected)"
            : "",
        },
    ...
    const icon = goal.icon
      ...
      : isEvent
        ? undefined
        : goal_entity_exceptionlist[goal.id.replace(/^minecraft:/, "")]
  ...
  form.show(player).then((response) => {
    ...
    if (response.selection < visibleGoals.length) {
      ...
      if (isEvent) {
        const realIndex = goalArray.findIndex(
          (goal) => goal.name === selectedGoal.name,
        );
        save_data[0].challenge.goal.event_pos = realIndex;
      } else {
...
function settings_time_zone(player, viewing_mode) {
  ...
  const TICKS = 24000,
    MILLIS_DAY = 86400000,
    START_OFFSET = 6 * 3600000;
  let now = new Date();

  let current_zone_index = timezone_list.findIndex(
    (zone) => zone.utc === save_data[0].utc,
  );
  ...
  if (viewing_mode == 0) {
    ...
    for (let index = start; index <= end; index++) {
      ...
      let total =
        (now.getHours() + zone.utc) * 3600000 +
        now.getMinutes() * 60000 +
        now.getSeconds() * 1000 +
        Math.floor(now.getMilliseconds() / 10) * 10;
      let adj =
        total - START_OFFSET < 0
          ? total - START_OFFSET + MILLIS_DAY
          : total - START_OFFSET;
...
function settings_main(player) {
  ...
  if (
    (!save_data[0].global.status ||
      (save_data[0].global.status && save_data[player_sd_index].op)) &&
    ((save_data[0].challenge.active && save_data[0].challenge.progress == 0) ||
      !save_data[0].challenge.active)
  ) {
    form.button(
      "Type\n§9" +
        timer_modes[
          save_data[save_data[0].global.status ? 0 : player_sd_index]
            .counting_type
        ].label,
      timer_modes[
        save_data[save_data[0].global.status ? 0 : player_sd_index]
  ...
  if (save_data[player_sd_index].op) {
    form.button(
      "Permission\n" +
        (() => {
          ...
          const names = save_data
            .slice(1)
            .sort((a, b) =>
              ids.includes(a.id) && !ids.includes(b.id)
                ? -1
                : ids.includes(b.id) && !ids.includes(a.id)
                  ? 1
                  : 0,
            )
            .map((e) => e.name);
          return names.length > 1
            ? names.slice(0, -1).join(", ") + " u. " + names[names.length - 1]
            : names.join(", ");
        })(),
      "textures/ui/op",
    );
    actions.push(() => {
      ...
      player.playMusic(
        translate_soundkeys("music.menu.settings.rights", player),
        { fade: 0.3, loop: true },
      );
    });
  }
  ...
  actions.push(() => {
    player.playMusic(
      translate_soundkeys("music.menu.settings.actionbar", player),
      { fade: 0.3, loop: true },
    );
  ...
  if (independent) {
    ...
    actions.push(() => {
      player.playMusic(
        translate_soundkeys("music.menu.settings.gestures", player),
        { fade: 0.3, loop: true },
      );
  ...
  if (save_data[player_sd_index].op == true) {
    if (form) {
      ...
      if (!zone) {
        ...
        zone_text =
          "Prob. " +
          ("Prob. " + zone.name.length > 30 ? zone.short : zone.name);
      } else {
    ...
    actions.push(() => {
      player.playMusic(
        translate_soundkeys("music.menu.settings.time_zone", player),
        { fade: 0.3, loop: true },
      );
  ...
  // Button 4: Fullbright
  form.button(
    "Fullbright\n" + (save_data[player_sd_index].fullbright ? "§aon" : "§coff"),
    save_data[player_sd_index].fullbright
  ...
  form.button(
    "Custom Sounds\n" +
      (save_data[player_sd_index].custom_sounds > 0
        ? save_data[player_sd_index].custom_sounds == 1
          ? "§aon"
          : "§aon (legacy)"
        : "§coff"),
    save_data[player_sd_index].custom_sounds > 0
  ...
  if (typeof gen === "number" && save_data[player_sd_index].op) {
    form.button(
      "Update\n" + gen_list[gen] + " -> " + version_info.version,
...
function settings_gestures(player) {
  ...
  available.forEach((gesture) => {
    ...
    const restricted =
      isOn &&
      configured_gestures[gesture].some((mode) => modeCounts[mode] === 1);
    if (restricted) {
...
function dictionary_about_version(player) {
  ...
  form.body(
    "Name: " +
      ...
      version_info.version +
      (Math.floor(Date.now() / 1000) >
      version_info.update_message_period_unix + version_info.unix
        ? " §a(update time)§r"
        : " (" + version_info.build + ")") +
      "\n" +
      "Release type: " +
      ["dev", "preview", "stable"][version_info.release_type] +
      "\n" +
      "Build date: " +
      ...
      "\n\n§7© 2022-" +
      build_date.year +
      " TheFelixLive. All rights reserved.",
  );

  if (
    version_info.changelog.new_features.length > 0 ||
    version_info.changelog.general_changes.length > 0 ||
    version_info.changelog.bug_fixes.length > 0
  ) {
...
function dictionary_contact(player, build_date) {
  ...
  save_data.push({
    dump_unix: Math.floor(Date.now() / 1000),
  ...
  form.title("Contact");
  form.body(
    "If you need want to report a bug, need help, or have suggestions to improvements to the project, you can reach me via these platforms:\n\n§l§5Github:§r github.com/TheFelixLive/Timer-V/issues\n\n§8Curseforge:§r curseforge.com/projects/1259478",
  );

  if (save_data[player_sd_index].op) {
    form.button(
      "Dump SD" + (version_info.release_type !== 2 ? "\nvia. privat chat" : ""),
    );
    actions.push(() => {
      player.sendMessage(
        "§l§7[§f" +
...
function dictionary_about_version_changelog(player, build_date) {
  ...
  bodyText +=
    `§7As of ${build_date.day}.${build_date.month}.${build_date.year} (` +
    getRelativeTime(Math.floor(Date.now() / 1000) - version_info.unix) +
    " ago)";
    ...
    .forEach((e) => {
      if (
        ...
        if (!goal_entity_blocklist.find((entity) => entity.id == id)) {
          ...
          form.button(
            { rawtext: [{ translate: "entity." + id + ".name" }] },
            icon,
          );
          actions.push(() => {
...
function debug_sd_editor(player, onBack, path = []) {
  ...
  if (Array.isArray(current)) {
    ...
    form.show(player).then((res) => {
      ...
      debug_sd_editor(player, returnToCurrentMenu, [...path, res.selection]);
    });
...
function settings_rights_main(player, came_from_settings) {
  ...
  newList.forEach((entry) => {
    ...
    if (isOnline) {
      ...
      displayName +=
        "\n§o(last seen " +
        getRelativeTime(Math.floor(Date.now() / 1000) - entry.last_unix) +
        " ago)§r";
    }
...
function settings_rights_data(viewing_player, selected_save_data) {
  let save_data = load_save_data();
  let selected_player = world
    .getAllPlayers()
    .find((player) => player.id == selected_save_data.id);
  let form = new ActionFormData();
  ...
  body_text +=
    "Name: " +
    selected_save_data.name +
    " (id: " +
    selected_save_data.id +
    ")\n";
  if (version_info.release_type === 0) {
    body_text +=
      "Language: " +
      ["English" /* Placeholder! */][selected_save_data.lang] +
      "\n";
  }
  ...
  if (selected_player) {
    ...
    body_text +=
      "Online: no §7(last seen " +
      getRelativeTime(
        Math.floor(Date.now() / 1000) - selected_save_data.last_unix,
      ) +
      " ago)§r\n";
  }
  ...
  if (
    selected_save_data.id == save_data[0].global.last_player_id &&
    save_data[0].challenge.active
  ) {
    body_text +=
  ...
  if (selected_save_data.id !== viewing_player.id) {
    ...
    if (selected_save_data.op) {
      ...
      actions.push(() => {
        let player_sd_index = save_data.findIndex(
          (entry) => entry.id === selected_save_data.id,
        );
      ...
      actions.push(() => {
        ...
        form.body(
          "Your are trying to add op advantages to " +
            selected_save_data.name +
            ". With them he would be able to:\n\n- Mange other and your OP status\n- Mange Timer modes\n- Mange save data\n- Could delete the timer!\n\nAre you sure you want to add them?\n ",
        );
        form.button1("");
        form.button2("§aMake op");
        form.show(viewing_player).then((response) => {
          if (response.selection == undefined) {
            return viewing_player.playMusic(
              translate_soundkeys("menu.close", viewing_player),
              { fade: 0.3 },
            );
          }
          if (response.selection == 1) {
...
function settings_rights_manage_sd(viewing_player, selected_save_data) {
  ...
  form.show(viewing_player).then((response) => {
    ...
    if (is_global) {
      const player_sd_index = save_data.findIndex(
        (entry) => entry.id === viewing_player.id,
      );
      ...
      shared_form.show(viewing_player).then((global_response) => {
        ...
        const reload = () => {
          save_data = load_save_data();
          selected_save_data = save_data.find(
            (e) => e.id === selected_save_data.id,
          );
          handle_data_action(
            is_reset,
            is_delete,
...
function handle_data_action(
  is_reset,
  is_delete,
  viewing_player,
  selected_save_data,
) {
  const selected_player = world
    .getAllPlayers()
    .find((p) => p.id === selected_save_data.id);
  if (is_reset) {
  ...
  if (is_delete) {
    if (selected_player) {
      const confirm_form = new MessageFormData()
        ...
        .body(
          `Are you sure you want to remove ${selected_player.name}'s save data?\nThey must disconnect from the world!`,
        )
        .button1("")
        ...
        if (confirm.selection === 1) {
          if (
            !world
              ...
              .runCommand(`kick ${selected_player.name}`).successCount
          ) {
            const host_form = new MessageFormData()
              .title("Host player information")
              .body(
                `${selected_player.name} is the host. To delete their data, the server must shut down. This usually takes 5 seconds`,
              )
              .button1("")
...
function settings_type(player) {
  ...
  timer_modes.forEach((button, index) => {
    if (
      typeof button.show_if === "function"
  ...
  form.show(player).then((response) => {
    ...
    if (
      (save_data[player_sd_index].counting_type === 0 ||
        save_data[player_sd_index].counting_type === 1) &&
      save_data[player_sd_index].time.do_count &&
...
function settings_type_info(player, response) {
  ...
  form.body(
    "Your " +
      (save_data[player_sd_index].counting_type == 1 ? "timer" : "stopwatch") +
      " is not paused! If you change now the mode to " +
      timer_modes[response.selection].label +
      " it will be paused!",
  );
  form.button1("");
...
function settings_actionbar(player) {
  ...
  actions.push(() => {
    ...
    player.playMusic(
      translate_soundkeys("music.menu.settings.actionbar.design", player),
      { fade: 0.3, loop: true },
    );
  });
  ...
  form.button(
    "Use actionbar\n" +
      (save_data[player_sd_index].visibility_setting
        ? save_data[player_sd_index].absolute_visibility !==
          save_data[player_sd_index].visibility_setting
          ? "§9dynamic"
          : "§aon"
        : "§coff"),
  ...
  if (save_data[player_sd_index].counting_type !== 3) {
    form.button(
      ...
      save_data[player_sd_index].time_day_actionbar
        ? "textures/ui/toggle_on"
        : "textures/ui/toggle_off",
    );
    actions.push(() => {
  ...
  if (
    save_data[player_sd_index].time_day_actionbar ||
    save_data[player_sd_index].counting_type == 3
  ) {
    if (!save_data[0].sync_day_time) {
      if (form) {
        form.button(
...
function design_template_ui(player) {
  ...
  let sortedDesigns = design_template
    ...
    .sort(
      (a, b) => (b.content === currentDesign) - (a.content === currentDesign),
    );

  let hasMatchingDesign = sortedDesigns.some(
    (design) =>
      JSON.stringify(design.content) === JSON.stringify(currentDesign),
  );
  ...
  sortedDesigns.forEach((design) => {
    ...
    if (
      JSON.stringify(design.content) === JSON.stringify(currentDesign) ||
      (!hasMatchingDesign && design.content === undefined)
    ) {
...
function design_preview(player, design, is_custom) {
  ...
  let ui_preview = apply_design(
    design.find((d) => d.type === "ui"),
    660822121.4,
  );
  let normal_preview = apply_design(
    design.find((d) => d.type === "normal"),
    660822121.4,
  );
  let paused_preview = apply_design(
    design.find((d) => d.type === "paused"),
    660822121.4,
  );

  let finished_preview = apply_design(
    design.find((d) => d.type === "finished"),
    660822121.4,
  );
  let day_preview = apply_design(
    design.find((d) => d.type === "day"),
    19395.9,
  );

  let screen_saver_preview = apply_design(
    design.find((d) => d.type === "screen_saver"),
    0,
  );

  form.body(
    "Here is a preview of your selected design. It shows all possible variable at ones.\n\nScreen saver:\n" +
  ...
  form.show(player).then((response) => {
    ...
    if (response.selection == 0) {
      if (is_custom) {
        ...
        save_data[player_sd_index].design = design_template.findIndex(
          (d) => d.content === design,
        );
      }
...
function render_live_actionbar(selected_save_data, do_update) {
  ...
  if (typeof selected_save_data.design === "number")
    selected_save_data.design =
      design_template[selected_save_data.design].content;
  ...
  return d1
    ? apply_design(d0, tv.value) +
        " §r§f| " +
        apply_design(d1, calcAB(true, selected_save_data.id, true).value)
    : apply_design(d0, tv.value);
}
...
function calcAB(update, id, dayFormat) {
      ...
      if (counting_type === 1) {
        ...
        if (timedata.timer == 0) {
          if (data[0].challenge.progress == 1) {
            if (
              data[0].challenge.goal.pointer == 2 &&
              data[0].challenge.goal.event_pos == 0
            ) {
              finished_cm_timer(1, [
                {
                  text: "You did it! You persevered through the whole time and reached your goal!",
                },
              ]);
            } else {
              finished_cm_timer(0, [
                {
                  text: "The challenge is over because you went out of time. Thanks for playing.",
                },
              ]);
            }
            return -1;
            ...
            if (data[0].global.status) {
              world.getAllPlayers().forEach((player) => {
                ...
                player.sendMessage(
                  "§l§4[§cCondition§4]§r The timer expired after " +
                    apply_design(
                      (typeof data[idx].design === "number"
                        ? design_template[data[idx].design].content
                        : data[idx].design
                      ).find((item) => item.type === "ui"),
                      timedata.last_value_timer - timedata.timer,
                    ) +
                    " and has been paused",
                );
                ...
                player.playSound(
                  translate_soundkeys("condition.expired", player),
                );
              });
            } else {
              let player = world
  ...
  if (counting_type === 3 || dayFormat) {
    if (data[idx].time_source === 1 || data[0].sync_day_time) {
      ...
      let now = new Date(),
        total =
          (now.getHours() + data[0].utc) * 3600000 +
          now.getMinutes() * 60000 +
          now.getSeconds() * 1000 +
          Math.floor(now.getMilliseconds() / 10) * 10,
        adj =
      ...
      if (
        data[0].sync_day_time &&
        update &&
        (!data[0].challenge.active ||
          (data[0].challenge.progress === 1 && data[0].time.do_count))
      ) {
...
async function update_loop() {
  while (true) {
    ...
    for (const player of world.getAllPlayers()) {
        ...
        if (
          ...
          let key =
            save_data[player_sd_index].counting_type == 1
              ? "timer"
              : "stopwatch";
          let delta = save_data[player_sd_index].counting_type
            ? 15 * 20
            : -15 * 20;
      ...
      if (
        save_data[player_sd_index].fullbright &&
        (!nightVision || nightVision.duration < 205)
      ) {
      ...
      if (save_data[player_sd_index].absolute_visibility == true) {
        player.onScreenDisplay.setActionBar(
          render_live_actionbar(
            save_data[player_sd_index],
            save_data[0].global.status ? false : true,
          ),
        );
      }
    }
